...
export default function generatePDF(json, recintos, unidades, formatDate, formatDateYYYYMMDD) {
  ...
  if (item.indicadorPartesMadeira === "S") {
    y += 4;
    drawAlertText("POSSUI MADEIRA");
  }
  y += 10;
  ...
  if (item.bloqueiosAtivos?.[0]) {
    drawStructuredList(item.bloqueiosAtivos, "bloqueios", recintos);
  } else {
    drawEmptyText();
  }
  y += 10;
  ...
  if (estoque) {
    ...
    drawField("Qtd", item.quantidadeVolumesConhecimento);
    if (estoque.pesoBrutoEstoque !== null) {
      drawField("Peso", `${estoque.pesoBrutoEstoque} Kg`);
    }
    if (estoque.recintoAduaneiro !== null) {
      drawField(
        "RA",
        `${estoque.recintoAduaneiro} — ${recintos[estoque.recintoAduaneiro] ?? "Não encontrado"}`
      );
    }
    drawField(
      "URF",
      `${estoque.unidadeRfb} — ${unidades[estoque.unidadeRfb] ?? "Não encontrado"}`
    );
    const situacaoCarregamento =
      estoque.situacaoCarregamento === "NA"
        ? "Não se aplica"
        : estoque.situacaoCarregamento === "D"
        ? "Descarregada"
  ...
  if (item.recepcoesComAvarias?.[0]) {
    drawStructuredList(item.recepcoesComAvarias, "avarias", recintos);
  } else {
    drawEmptyText();
  }

  const nomeArquivo = `consulta_${item.ruc ?? item.identificacao ?? "relatorio"}.pdf`;
